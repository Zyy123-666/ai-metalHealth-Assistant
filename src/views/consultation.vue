...
const createNewFrontendSession = async () => {
    // 创建一个新的会话对象
    const newSession = {
        sessionId: `temp_${Date.now()}`, // 使用时间戳作为临时ID
        status: 'TEMP',
        sessionTitle: '新对话',
    }
...
const sendMessage = () => {
    ...
    if (isAiTyping.value) {
        ElMessage.success('请稍等，AI助手正在输入中...')
        return
    }
...
const startNewSession = (message) => {
    ...
    if (currentSession.value.sessionTitle === '新对话') {
        // 如果是新对话，使用当前时间作为标题
        sessionParams.sessionTitle = `萤火AI助手-${new Date().toLocaleTimeString()}`
    } else {
        // 如果是会话记录过来的，继续使用之前的标题
...
const handleSessionClick = (session) => {
    ...
    // 更新当前对话数据
    const sessionData = {
        sessionId: "session_" + session.id, // 使用时间戳作为临时ID
        status: 'ACTIVE',
...
// 用户和AI轮发信息，处理换行
const formMessageContent = (content) => {
            ...
            <div class="emotion-garden">
                ...
                <div class="warm-tips">
                    <div class="emotion-status-text">
                        <span class="status-label">今日感受</span>
                        <span class="status-emotion">{{ currentEmotion.isNegative ? '糟糕' : '良好' }}</span>
                    </div>
        ...
        <div class="chat-main">
                ...
                <div class="message-item ai-message" v-if="messages.length === 0">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>你好！我是萤火心理助手，很高兴为你提供心理健康方面的支持和帮助。你今天怎么样，有什么要分享的吗？</p>
                        </div>
            ...
            <div class="chat-input">
                <div class="input-container">
                    <el-input v-model="userMessage" placeholder="请输入你想交流的信息..." type="textarea" :rows="3"
